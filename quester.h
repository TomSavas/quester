...
{
    {NULL, NULL, NULL, 0}, //QUESTER_BUILTIN_AND_TASK, not implemented yet
    {NULL, NULL, NULL, 0}, //QUESTER_BUILTIN_OR_TASK, not implemented yet
...
void quester_run(struct quester_context *q_ctx)
{
    const int starting_tracked_node_count = q_ctx->tracked_node_count;
    for (int i = 0; i < starting_tracked_node_count; i++)
    {
        int node_id = q_ctx->tracked_node_ids[i];
        int node_index = quester_find_index(q_ctx, node_id);
        if (node_index == -1)
            continue;
        struct node *node = &q_ctx->all_nodes[node_index].node;

        void *node_data = q_ctx->user_data + quester_find_user_data_offset(q_ctx, node_id);
        if (!quester_node_implementations[node->type].is_completed(node_data))
            continue;

        q_ctx->tracked_node_ids[i] = q_ctx->tracked_node_ids[--q_ctx->tracked_node_count];
        for (int j = 0; j < node->out_node_count; j++)
        {
            int dup = 0;
            for (int k = 0; k < q_ctx->tracked_node_count; k++)
            {
                if (q_ctx->tracked_node_ids[k] == node->out_node_ids[j])
                {
                    dup = 1;
                    break;
                }
            ...
            if (!dup)
            {
                int newly_tracked_node_id = quester_find_index(q_ctx, node->out_node_ids[j]);
                struct node *newly_tracked_node = &q_ctx->all_nodes[newly_tracked_node_id];
                void *newly_tracked_node_data = q_ctx->user_data + quester_find_user_data_offset(q_ctx, newly_tracked_node_id);
                quester_node_implementations[newly_tracked_node->type].on_start(newly_tracked_node_data);

                q_ctx->tracked_node_ids[q_ctx->tracked_node_count++] = node->out_node_ids[j];
            }
        }
    }
}
...
void quester_fill_with_test_data(struct quester_context *ctx)
{
    ...
    ctx->all_nodes[ctx->node_count].editor_node.x = 300;
    ctx->all_nodes[ctx->node_count].editor_node.y = 300;
    *(struct timer_task*)(ctx->user_data + quester_find_user_data_offset(ctx, 0)) = (struct timer_task){0, 100};
    ...
    ctx->all_nodes[ctx->node_count].editor_node.x = 500;
    ctx->all_nodes[ctx->node_count].editor_node.y = 300;
    *(struct timer_task*)(ctx->user_data + quester_find_user_data_offset(ctx, 1)) = (struct timer_task){0, 100};
    ...
    ctx->all_nodes[ctx->node_count].editor_node.x = 700;
    ctx->all_nodes[ctx->node_count].editor_node.y = 300;
    *(struct test_task*)(ctx->user_data + quester_find_user_data_offset(ctx, 2)) = (struct test_task){"this is mission 02\0"};
    ...
    ctx->all_nodes[ctx->node_count].editor_node.x = 900;
    ctx->all_nodes[ctx->node_count].editor_node.y = 300;
    *(struct timer_task*)(ctx->user_data + quester_find_user_data_offset(ctx, 3)) = (struct timer_task){0, 100};
    ...
    ctx->all_nodes[ctx->node_count].editor_node.x = 1100;
    ctx->all_nodes[ctx->node_count].editor_node.y = 200;
    *(struct timer_task*)(ctx->user_data + quester_find_user_data_offset(ctx, 4)) = (struct timer_task){0, 100};
    ...
    ctx->all_nodes[ctx->node_count].editor_node.x = 1100;
    ctx->all_nodes[ctx->node_count].editor_node.y = 400;
